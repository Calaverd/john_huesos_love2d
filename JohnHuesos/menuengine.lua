...
-- Constructor
function menuengine.new(x, y, w, font, space)
    local self = {}
        ...
        for i=1,#self.entries do
            if not self.entries[i].disabled then
                ...
                if self.cursor == i and #self.entries[i].text > 0 then
                    ...
                    love.graphics.printf(self.entries[i].symbolSelectedBegin..self.entries[i].text..self.entries[i].symbolSelectedEnd,self.entries[i].x,self.entries[i].y,self.w,"center")
                else
                    love.graphics.setColor(self.entries[i].colorNormal)
                    love.graphics.printf(self.entries[i].normalSelectedBegin..self.entries[i].text..self.entries[i].normalSelectedEnd,self.entries[i].x,self.entries[i].y,self.w,"center")
                end
            end
        end
